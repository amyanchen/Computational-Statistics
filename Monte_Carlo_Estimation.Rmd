...
#### Estimate $\int_1^3 \frac{x}{(1+x^2)^2}\; dx$
Let $f(x) = \frac{1}{2}$ and $h(x) = 2 \frac{x}{(1+x^2)^2}$.  
Then $\int_1^3 2 *\frac{1}{2} \frac{x}{(1+x^2)^2}\; dx$  
$= E_{f}[2\frac{x}{(1+x^2)^2}\ ], f$~$U(1,3)$ so $f(x) = \frac{1}{2}\ $
```{r}
h <- function(x) {
  2*x/(1+x^2)^2;
}
...
#### Estimate $\int_{-\infty}^{\infty} x^2e^{-x^2} \; dx$
Let $f(x) = \frac{1}{\sqrt{\pi}}e^{-x^2}$ and $h(x) = \sqrt{\pi}x^2$.  
Then $\int_{-\infty}^{\infty} \sqrt{\pi} \frac{1}{\sqrt{\pi}}e^{-x^2}x^2 \; dx$  
$= E_{f}[\sqrt{\pi}x^2], f$~$N(0,\sqrt{\frac{1}{2}})$ so $f(x) = \frac{1}{\sqrt{\pi}}e^{-x^2}$
```{r}
h <- function(x) {
  sqrt(pi)*x^2;
}
...
For both of the above examples, a sample size of 100,000 was selected. We shall now examine how the estimation changes as sample size increases.
